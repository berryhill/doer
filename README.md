...
Optional overrides: `--profile NAME` selects a Hermes profile, `--provider NAME` selects a provider for both chats, `--sol MODEL` selects the implementation model (otherwise ambient), and `--luna MODEL` replaces the default `gpt-6-luna` diagnosis/final-assessment model. Profile and provider remain ambient unless supplied. These options do not configure credentials or guarantee access to the selected models.
